...
def turso_execute(statements: list[dict]) -> dict:
    ...
    # Retry hingga 3x untuk timeout dan 502/503
    import time
    last_err = None
    for attempt in range(3):
        try:
            ...
            if e.code in (502, 503, 429) and attempt < 2:
                wait = (attempt + 1) * 5
                print(f"\n  ⏳ Turso HTTP {e.code}, retry {attempt+1}/3 dalam {wait}s...")
                time.sleep(wait)
                last_err = RuntimeError(f"Turso HTTP {e.code}: {body[:300]}")
                continue
            raise RuntimeError(f"Turso HTTP {e.code}: {body[:300]}") from e
        except (TimeoutError, OSError) as e:
            if attempt < 2:
                wait = (attempt + 1) * 8
                print(f"\n  ⏳ Timeout/connection error, retry {attempt+1}/3 dalam {wait}s...")
                time.sleep(wait)
                last_err = e
                continue
            raise RuntimeError(f"Timeout setelah 3 percobaan: {e}") from e
    raise last_err
...
def turso_batch(stmts: list[dict], batch_size: int = 100):
    ...
    for i in range(0, total, batch_size):
        ...
        try:
            ...
            errors += 1
            # Lanjut ke batch berikutnya, jangan stop
        sent += len(chunk)
        pct   = sent * 100 // total
...
def detect_village_events(today_rows: list, yesterday_villages: dict, snap_date: str) -> list:
    ...
    for vid, prev in yesterday_villages.items():
        ...
        if curr is None:
            ...
            # prev_pid == 0 skip (unowned), tapi prev_pid == 1 (Natars) tetap diproses
            NATARS_ID = 1
            if prev_pid != curr_pid and (prev_pid != 0):
                if prev_pid == NATARS_ID and curr_pid == 0:
                    pass  # Natars → unowned, skip
                else:
